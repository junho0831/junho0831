...
데이터 생성부터 장애 복구까지 이어지는 운영 흐름을 견고하게 설계하며, 운영 안정성과 데이터 무결성을 시스템으로 보장합니다.
...
<table>
  <tr align="center">
    <td width="20%">
      ...
      <sub>처리 시간</sub><br>
      <h3>30.6% 단축</h3>
      <sub>4,175초 → 2,896초</sub>
    </td>
    <td width="20%">
      <b>장애 분석 리드타임</b><br><br>
      <sub>원인 규명 속도</sub><br>
      <h3>70% 단축</h3>
      <sub>40분 → 12분</sub>
    </td>
    <td width="20%">
      <b>CI/CD 자동 배포</b><br><br>
      <sub>배포 실패율</sub><br>
      <h3>0% 달성</h3>
      <sub>릴리스 리드타임 58% 단축</sub>
    </td>
    <td width="20%">
      <b>사내 RAG 검색</b><br><br>
      <sub>질의응답 정확도</sub><br>
    ...
    <td width="20%">
      <b>데이터 정합성</b><br><br>
      <sub>월간 불일치 이슈</sub><br>
      <h3>0건 달성</h3>
      <sub>Admin 재처리 자동화</sub>
    </td>
  </tr>
</table>
...
### 엔셀 | 백엔드 개발자
`2025.01 ~ 현재` | `Data/Batch Backend`

#### 1. Prism - Airflow 대용량 데이터 처리 자동화
- **기술 스택**: `Python` `Airflow` `PostgreSQL` `FTP`
- **성과**: 약 1,973만 건 처리 시간 **30.6% 단축** (`4,175초 → 2,896초`)
- **해결**: PostgreSQL Range Partition 청크 COPY 스트리밍 & 파이프라인 오버랩(Pipelining) 실행
- **안정성**: DB Commit 후 원본 삭제 순서 고정 및 `source_file` Unique Constraint + UPSERT로 중복 적재 원천 차단

#### 2. DataForge - Spring 검색 / 인증 / 관리 API
- **기술 스택**: `Java` `Spring Boot` `PostgreSQL` `Redis` `Elasticsearch` `JWT` `OAuth2`
- **성과**: Elasticsearch 장애 발생 시 무중단 **DB Fallback 자동 전환** (동일 응답 포맷 일원화)
- **보안/운영**: Refresh Token을 **Redis TTL** 기준으로 단일화, 운영자 수동 DB 접근을 없앤 **Admin 재처리 API** 구축

#### 3. SMIP - 공통 예외 처리 계층 구축 및 회귀 테스트
- **기술 스택**: `Java` `Spring Boot` `Vue.js` `JUnit5` `Mockito`
- **성과**: 장애 분석 리드타임 **70% 단축** (`40분 → 12분`), 동일 오류 재발률 **30% 감소**
- **해결**: 표준 오류 응답 포맷(Global Exception Handler) 통합 및 장애 케이스 JUnit5/Mockito 회귀 테스트 코드화

---

### (주) 헥토 | AI개발팀 백엔드 개발자
`2023.10 ~ 2024.07` | `Backend & CI/CD`

#### 1. KMS - CI/CD 배포 자동화
- **기술 스택**: `Java` `Spring Boot` `GitLab CI/CD` `Docker` `Nginx`
- **성과**: 릴리스 리드타임 **58% 단축** (`1시간 → 25분`), 배포 실패율 **0% 달성** (`5% → 0%`)
- **해결**: GitLab CI/CD와 Docker 기반 빌드·테스트·배포 자동화 표준화 및 Nginx Staging 환경 구축

#### 2. SmartQ - 사내 지식 RAG 검색 API
- **기술 스택**: `Java` `Spring Boot` `OpenAI API` `LangChain` `FastAPI`
- **성과**: 질의응답 정확도 **80% 향상** (`60% → 80%`), 답변 대기 시간 **80% 단축** (`5분 → 1분`, 주당 12시간 절감)
- **해결**: 키워드 단순 매칭 한계를 해결하기 위해 LangChain 기반 사내 지식 RAG API 구축

---

### (주) 헥토이노베이션 | 마이데이터팀 사원
`2022.08 ~ 2023.10` | `Batch & Data`

#### SafeCash - 정기 배치 및 Admin 재처리 API
- **기술 스택**: `Java` `Spring Boot` `MyBatis` `MySQL` `Crontab`
- **성과**: 월간 데이터 정합성 이슈 **0건 달성** (`월 3건 → 0건`)
- **해결**: Crontab 정기 배치 자동화 및 개발자 수동 DB 수정을 대체하는 운영자 Admin UI/API 재처리 시스템 구축
...
- **이벤트 무결성**: DB Outbox 패턴과 `FOR UPDATE SKIP LOCKED` 적용으로 Redis Pub/Sub 순단 시에도 이벤트 유실 방지
